...
async function captureShokzHomeBanners(client, outputPath, viewport, captureContext = {}) {
  ...
  const compositeResult = await composeShokzHomeModuleCompositeCaptures({
    ...
    definition: {
      key: "banner",
      sectionLabel: "Banner",
      title: captures[0]?.sectionTitle || "Banner"
    },
    outputPath,
    viewport
  });
  if (compositeResult.captures.length) {
    return {
      ...
      bannerInfo: {
        ...
        savedCount: compositeResult.captures.length,
        stateCount: plan.count,
        duplicateCount: duplicates.length,
        duplicates,
        status: compositeResult.warnings.length ? "warning" : "ok",
...
async function composeShokzHomeModuleCompositeCaptures({
  captures,
  definition,
  outputPath,
  viewport
  ...
  const stateCapturesWithBuffers = [];
  let mainBuffer = null;
  ...
  try {
    ...
    warnings.push({
      sectionKey: definition.key,
      sectionLabel: definition.sectionLabel,
      message: `Could not read main homepage screenshot for ${definition.sectionLabel || definition.key} composition: ${error.message}`
    });
    return {
      width: Number(viewport.width || 0) || 393,
      height: Number(viewport.height || 0) || 852,
  ...
  for (const capture of orderedCaptures) {
    try {
      stateCapturesWithBuffers.push({ ...capture, buffer: await fs.readFile(capture.outputPath) });
    } catch (error) {
      warnings.push({
        sectionKey: definition.key,
        sectionLabel: definition.sectionLabel,
        stateLabel: capture.stateLabel,
        message: `Could not read ${definition.sectionLabel || definition.key} screenshot for composition: ${error.message}`
      });
    }
  }

  if (!stateCapturesWithBuffers.length) {
    warnings.push({
      sectionKey: definition.key,
      sectionLabel: definition.sectionLabel,
      message: `Could not compose ${definition.sectionLabel || definition.key} screenshots because no states were captured.`
    });
    return {
      width: Number(viewport.width || 0) || 393,
      height: Number(viewport.height || 0) || 852,
  ...
  let composite;
  try {
    composite = composeShokzHomeModuleComposite({
      mainCapture: { buffer: mainBuffer },
      stateCaptures: stateCapturesWithBuffers,
      viewport,
      sourceKind: `home-${definition.key || "module"}`
    });
  } catch (error) {
    warnings.push({
      sectionKey: definition.key,
      sectionLabel: definition.sectionLabel,
      message: `Could not compose ${definition.sectionLabel || definition.key} screenshots: ${error.message}`
    });
    return {
      width: Number(viewport.width || 0) || 393,
      height: Number(viewport.height || 0) || 852,
  ...
  const categoryKey = safeFilePart(definition.key || definition.sectionLabel || "module");
  const categoryLabel = definition.sectionLabel || definition.key || "Module";
  const visibleProducts = definition.key === "product-showcase"
    ? homeProductShowcaseVisibleProducts(stateCapturesWithBuffers)
    : [];
  const visibleItems = composite.stateCaptures.map((item) => ({
    key: item.capture.coverageKey || item.capture.logicalSignature || item.capture.bannerSignature || item.capture.stateLabel || `${categoryKey}-${item.index + 1}`,
    label: item.capture.stateLabel || item.capture.label || `${categoryLabel} ${item.index + 1}`,
    text: item.capture.sectionState?.text || item.capture.bannerState?.text || "",
    tabLabel: item.capture.tabLabel || null,
    tabIndex: item.capture.tabIndex || null,
  ...
  const stateLabel = `${categoryLabel} Composite`;
  const width = composite.width;
  const height = composite.height;
  const windowSignature = JSON.stringify({
    sectionKey: definition.key,
    states: stateCapturesWithBuffers.map((item) =>
      item.logicalSignature || item.bannerSignature || item.windowSignature || item.stateLabel
    )
  }).slice(0, 1800);
  ...
  const compositeCapture = {
    ...
    productIndex: null,
    variantKey: null,
    variantLabel: `${stateCapturesWithBuffers.length} carousel states`,
    variantOptions: null,
    ...
    captureValidation: {
      ...
      attempts: stateCapturesWithBuffers.map((item) => item.captureValidation).filter(Boolean)
    },
    clip: {
      x: 0,
      y: 0,
    ...
    scrollInfo: {
      ...
      pageCount: stateCapturesWithBuffers.length
    },
    ...
    visibleProducts: visibleProducts.length ? visibleProducts : null,
    itemCount: visibleItems.length,
    visibleItemCount: visibleItems.length,
    visibleItems,
    itemRects,
    windowSignature,
    sectionState: {
      text: combinedRelatedCaptureText(stateCapturesWithBuffers),
      textBlocks: combinedRelatedCaptureTextBlocks(stateCapturesWithBuffers),
      images: combinedRelatedCaptureImages(stateCapturesWithBuffers),
      activeIndex: 1,
      interactionState: "default",
      categoryKey,
      categoryLabel,
...
function composeShokzHomeTopbarComposite({ mainCapture, topbarCaptures = [], viewport = {} }) {
  return composeShokzHomeModuleComposite({
    mainCapture,
    stateCaptures: topbarCaptures,
    viewport,
    sourceKind: "home-topbar"
  });
}

function composeShokzHomeModuleComposite({ mainCapture, stateCaptures = [], viewport = {}, sourceKind = "home-module" }) {
  const mainImage = decodePng(mainCapture.buffer);
  const gutter = 24;
  const itemGap = 18;
  const outerPad = 24;
  const items = stateCaptures
    .map((capture, index) => {
...
function homeModuleCompositeRows(items, itemGap = 18, maxRightWidth = Infinity) {
  ...
  for (const item of [...items].sort((a, b) =>
    ...
    const row = sourceRows.find((candidate) => Math.abs(candidate.y - item.y) <= tolerance);
    if (row) {
...
function combinedRelatedCaptureText(captures) {
  ...
  const seen = new Set();
  for (const capture of captures || []) {
    const text = String(capture.sectionState?.text || capture.bannerState?.text || "").replace(/\s+/g, " ").trim();
...
function combinedRelatedCaptureTextBlocks(captures) {
  ...
  const seen = new Set();
  for (const capture of captures || []) {
    for (const block of capture.sectionState?.textBlocks || capture.bannerState?.textBlocks || []) {
...
function combinedRelatedCaptureImages(captures) {
  ...
  const seen = new Set();
  for (const capture of captures || []) {
    for (const image of capture.sectionState?.images || capture.bannerState?.images || capture.images || []) {
...
async function captureShokzComparisonProductMapSection(client, outputPath, captureContext, definition) {
  ...
  const legacyMapSelected = targetFilter && comparisonProductMapMatchesFilter(outputPath, definition, plan, targetFilter);
  const productMatchesTargetFilter = (product) => comparisonProductMatchesFilter(definition, product, targetFilter);
  const targetProducts = targetFilter
    ? plan.products.filter(productMatchesTargetFilter)
    : plan.products;
  const productsToCapture = targetProducts.length
    ? targetProducts
    : legacyMapSelected
      ? [activeProductForTrack(1, plan.products[0], "openrunpro2")]
      : [];
  if (targetFilter && !productsToCapture.length) {
    throw new Error(`Could not match the requested ${definition.sectionLabel} product screenshot.`);
  }

  const captures = [];
  for (const [index, product] of productsToCapture.entries()) {
    const productCapture = await captureShokzComparisonProductComposite(client, {
      outputPath,
      captureContext,
      definition,
      viewport,
      product,
      productIndex: product.stateIndex || product.productIndex || index + 1,
      targetProductCount: productsToCapture.length,
      useRelatedStateOutputPath: Boolean(targetFilter && captureContext.relatedStateOutputPath && productsToCapture.length === 1)
    });
    warnings.push(...productCapture.warnings);
    if (productCapture.capture) {
      captures.push(productCapture.capture);
    }
  }

  warnings.push(...relatedSectionCoverageWarnings(definition, productsToCapture, captures));
  ...
  return {
    ...
    captures,
    warnings,
    expectedCount: productsToCapture.length,
    capturedCount: captures.length
  };
}

async function captureShokzComparisonProductComposite(client, {
  outputPath,
  captureContext,
  definition,
  viewport,
  ...
  try {
    ...
    return { capture: null, warnings };
  }

  const activation = await activateShokzComparisonProductSelection(client, { ...pairedProduct, trackIndex: 1 });
  if (!activation.ok) {
    warnings.push({
      sectionKey: definition.key,
      sectionLabel: definition.sectionLabel,
      stateLabel: productLabel,
      message: activation.reason || `Could not select ${productLabel}.`
    });
    return { capture: null, warnings };
  }
  const secondaryActivation = await activateShokzComparisonProductSelection(client, { ...pairedProduct, trackIndex: 2 });
  if (!secondaryActivation.ok) {
    warnings.push({
      sectionKey: definition.key,
      sectionLabel: definition.sectionLabel,
      stateLabel: productLabel,
      message: secondaryActivation.reason || `Could not select ${productLabel} in the second comparison column.`
    });
    return { capture: null, warnings };
  }
  const ready = await waitForShokzComparisonProductSelection(client, { ...pairedProduct, trackIndex: 1 });
  const secondaryReady = await waitForShokzComparisonProductSelection(client, { ...pairedProduct, trackIndex: 2 });
  if (!ready.ok || !secondaryReady.ok) {
    warnings.push({
      sectionKey: definition.key,
      sectionLabel: definition.sectionLabel,
      stateLabel: productLabel,
      message: ready.reason || secondaryReady.reason || `Could not confirm ${productLabel} in both comparison columns.`
    });
    return { capture: null, warnings };
  }
  ...
  const variantItems = compositeCaptures.map((item) => ({
    ...
    label: item.state.variantLabel || item.state.stateLabel,
    text: item.current.text || item.state.stateLabel || "",
    productKey: item.state.productKey || productKey,
    productLabel: item.state.productLabel || productLabel,
...
function comparisonProductMatchesFilter(definition, product, filter) {
  ...
  const tileKey = normalize(filter.tileKey);
  const productValues = [
    product.productKey,
    product.productLabel,
    product.fileId,
    product.logicalSignature,
    [definition.key, product.productKey, "product-map"].filter(Boolean).join("|")
  ...
  const tileParts = new Set(tileKey.split(/[^a-z0-9-]+/).filter(Boolean));
  return Boolean(tileKey && (productValues.includes(tileKey) || (productKey && tileParts.has(productKey)))) ||
    filterValues.some((filterValue) => productValues.includes(filterValue));
}
    ...
    expression: `(async () => {
      ...
      const readSizeOptions = () => {
        ...
        const namedBlocks = optionBlocks.filter((element) => {
          ...
          return /^size\\s*:?$/i.test(optionName);
        });
        const roots = namedBlocks;
      ...
      const activeSizeKey = () => {
        const active = readSizeOptions().find((item) => item.active) || null;
        return active?.sizeKey || "";
      };
      const activeColorIndex = () => {
        const swatches = readColorSwatches();
        const active = swatches.find((item) => item.active) || null;
        return active?.colorIndex || null;
      };
      ...
      const defaultSizeSlot = initialSizes.find((size) => size.active) || initialSizes[0] || null;
      const selectSize = async (sizeSlot) => {
        if (!sizeSlot || sizeSlot.isDefault) {
          return sizeSlot || { sizeLabel: "", sizeKey: "default", sizeIndex: 1, isDefault: true };
        }
      ...
      const resetDefaultSize = async () => {
        if (!defaultSizeSlot) return true;
        const size = await selectSize(defaultSizeSlot);
        return !size?.sizeKey || activeSizeKey() === size.sizeKey;
      };
      await resetDefaultSize();
      ...
      const colorSlots = initialSwatches.length
        ...
        : [{
          colorIndex: 1,
          variantId: activeVariantId() || productKey + "-default"
        }];
      const states = [];
      const seenStates = new Set();
      for (const color of colorSlots) {
        await resetDefaultSize();
        const defaultSwatches = readColorSwatches();
        const defaultSwatch = defaultSwatches[color.colorIndex - 1] ||
          defaultSwatches.find((item) => item.variantId === color.variantId) ||
          null;
        if (defaultSwatches.length && !defaultSwatch) {
          continue;
        }
        if (defaultSwatch?.element) {
          await clickElement(defaultSwatch.element, 180);
        }
        if (defaultSwatches.length && activeColorIndex() !== color.colorIndex) {
          continue;
        }
        const currentSizeOptions = readSizeOptions();
        const sizeSlots = currentSizeOptions.length ? currentSizeOptions.map((size) => ({
        ...
        for (const sizeSlot of sizeSlots) {
          const size = await selectSize(sizeSlot);
          if (!size?.isDefault && size?.sizeKey && activeSizeKey() !== size.sizeKey) {
            continue;
          }
          if (defaultSwatches.length && activeColorIndex() !== color.colorIndex) {
            continue;
          }
          const resolvedVariantId = activeVariantId() || defaultSwatch?.variantId || color.variantId || [productKey, "color", color.colorIndex].join("-");
          ...
          states.push({
            ...
            colorControlId: defaultSwatch?.variantId || color.variantId || "",
            sizeLabel: size.sizeLabel || "",
            sizeKey: size.sizeLabel ? size.sizeKey : "",
            sizeIndex: size.sizeLabel ? size.sizeIndex : null,
            variantOptions: [
              { name: "Color", value: colorLabel, controlId: defaultSwatch?.variantId || color.variantId || "", index: color.colorIndex },
      ...
      states.sort((a, b) =>
        Number(a.colorIndex || 0) - Number(b.colorIndex || 0) ||
        Number(a.sizeIndex || 0) - Number(b.sizeIndex || 0) ||
        String(a.variantKey || "").localeCompare(String(b.variantKey || ""), "en")
      );
      states.forEach((state, index) => {
        state.variantIndex = index + 1;
      });
      return {
        ok: true,
        states,
...
async function captureShokzComparisonProductVariantPairCard(client, definition, state, viewport, options = {}) {
  ...
  await prepareForScreenshotCapture(client, {
    rounds: 2,
    shokzKnownPopups: true,
    stage: `before Shokz ${definition.sectionLabel} paired product color screenshot capture`
  });
  await dismissShokzKnownPopupsBeforeScreenshot(client, { rounds: 2 });
  await dismissObstructions(client, { rounds: 2 });

  let current = await readShokzComparisonProductVariantPairState(client, definition, state);
  if (!current.ok) {
    return {
      ok: false,
      reason: current.reason || `Could not read ${state.stateLabel}.`
  ...
  if (!clip) {
    return {
      ok: false,
      reason: `Could not compute a valid paired product-card crop for ${state.stateLabel}.`
    };
  }
    ...
    const screenshotCapture = await captureScreenshotWithValidation(client, () => ({
      ...
      beforeAttempt: async ({ attempt }) => {
        ...
        await prepareForScreenshotCapture(client, {
          rounds: 2,
          shokzKnownPopups: true,
          stage: `retrying Shokz ${definition.sectionLabel} paired product color screenshot capture`
        });
        await dismissShokzKnownPopupsBeforeScreenshot(client, { rounds: 2 });
        await dismissObstructions(client, { rounds: 2 });
        current = await readShokzComparisonProductVariantPairState(client, definition, state);
        if (!current.ok) {
          throw new Error(current.reason || `Could not reread ${state.stateLabel}.`);
        }
        clip = normalizeRelatedClip(current.clip, viewport);
        if (!clip) {
          throw new Error(`Could not compute a valid paired product-card crop for ${state.stateLabel}.`);
        }
      }
    });
...
async function activateShokzComparisonProductVariantPairState(client, state) {
  const trackIndexes = Array.isArray(state?.trackIndexes) && state.trackIndexes.length
    ? state.trackIndexes.map((trackIndex) => Number(trackIndex || 0)).filter(Boolean)
    : [1, 2];
  for (const trackIndex of trackIndexes) {
    const activation = await activateShokzComparisonProductVariantState(client, {
      ...state,
      trackIndex
    });
    if (!activation.ok) {
    ...
    const result = await client.send("Runtime.evaluate", {
      expression: `(() => {
        ...
        const checks = trackIndexes.map((trackIndex) => {
          ...
          const swatches = Array.from(content?.querySelectorAll?.(".product-color-swatch-wrapper .swatch[data-id]") || [])
            .filter((element) => !element.classList.contains("hidden") && visible(element));
          const activeSwatch = swatches.find((element) => element.classList.contains("active")) || null;
          ...
          const activeSwatchIndex = activeSwatch ? swatches.indexOf(activeSwatch) + 1 : null;
          const swatchIdOk = !state.variantId ||
            activeSwatchId === state.variantId ||
            (state.colorControlId && activeSwatchId === state.colorControlId);
          const swatchOk = state.colorIndex
            ? activeSwatchIndex === state.colorIndex
            : swatchIdOk;
          const imageOk = state.colorIndex
            ? true
            : (!state.variantId || !activeImage || activeImageId === state.variantId);
          const desiredSizeKey = keyPart(state.sizeKey || state.sizeLabel);
      ...
      const cards = Array.from(document.querySelectorAll(selectors.join(", ")))
        ...
        .map((element) => {
          ...
          return {
            element,
            rect,
            absoluteY: Math.max(0, Math.round(window.scrollY + rect.top))
          };
        })
        .filter((entry) => Number.isFinite(targetAnchorY) || entry.rect.top > -80)
        .sort((a, b) =>
...
async function readShokzComparisonProductVariantPairState(client, definition, state) {
  const result = await client.send("Runtime.evaluate", {
    expression: `(() => {
      const definition = ${JSON.stringify({
        key: definition?.key || "",
      ...
      const activeContent = (root) => root?.querySelector?.(".content.active[data-handle]") || null;
      const handleMatches = (content) => {
        const handle = clean(content?.getAttribute?.("data-handle"), 100).toLowerCase();
        return content && (!state.productKey || handle === clean(state.productKey, 100).toLowerCase());
      };
      ...
      if (completeTracks.length !== trackIndexes.length) {
        return { ok: false, reason: "Could not read paired product cards for " + (state.productLabel || state.productKey || "") + "." };
      }

      const scrollTarget = completeTracks[0].selector || completeTracks[0].card;
      scrollTarget?.scrollIntoView?.({ block: "start", inline: "center" });

      const clipElements = completeTracks
        .flatMap((entry) => [entry.selector, entry.card])
        .filter((element, index, list) => element && visible(element) && list.indexOf(element) === index);
      const rects = clipElements.map((element) => element.getBoundingClientRect());
      const left = Math.min(...rects.map((rect) => rect.left));
      const top = Math.min(...rects.map((rect) => rect.top));
      const right = Math.max(...rects.map((rect) => rect.right));
      const bottom = Math.max(...rects.map((rect) => rect.bottom));
      const viewportWidth = window.innerWidth || document.documentElement.clientWidth || 393;
      const clipX = Math.max(0, Math.floor(left - 6));
      const clipY = Math.max(0, Math.floor(window.scrollY + top - 6));
      const clipRight = Math.min(viewportWidth, Math.ceil(right + 6));
      const clipWidth = Math.max(1, clipRight - clipX);
      const clipHeight = Math.max(220, Math.ceil(bottom - top + 12));
      const intersectsClip = (rect) =>
        rect.right >= left - 6 &&
        rect.left <= right + 6 &&
        rect.bottom >= top - 6 &&
        rect.top <= bottom + 6;
      ...
      const textElements = uniqueElements(clipElements.flatMap((element) =>
        [element, ...Array.from(element.querySelectorAll("h1, h2, h3, h4, p, a, button, span, strong, li, label, legend"))]
      ));
      const textBlocks = textElements
        ...
        .filter((block) => block.text && intersectsClip(block.rect))
        ...
        .slice(0, 100);
      const images = uniqueElements(clipElements.flatMap((element) => Array.from(element.querySelectorAll("img"))))
        .filter((image) => visible(image))
        .map((image) => ({
          src: image.currentSrc || image.src || image.getAttribute("data-src") || "",
          alt: clean(image.alt, 160),
        ...
        .filter((image) => image.src)
        .slice(0, 24);
      const visibleItems = completeTracks.map((entry) => {
      ...
      return {
        ...
        windowSignature: JSON.stringify({
          product: state.productKey,
          variant: state.variantKey,
          tracks: visibleItems.map((item) => item.trackIndex + ":" + item.variantId),
          texts: textBlocks.slice(0, 24).map((block) => block.text),
          images: images.map((image) => image.src)
        }).slice(0, 1800)
      };
  ...
  return result?.result?.value || { ok: false, reason: `Could not read ${state?.stateLabel || "comparison product color"} paired state.` };
}
